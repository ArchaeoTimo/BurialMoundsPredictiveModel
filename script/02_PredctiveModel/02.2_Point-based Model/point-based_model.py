...
if len(sys.argv) < 4:
    raise SystemExit("usage: python point-based_model.py <run_idx> <gdb_path> <metrics_folder>")
...
# names for this run
prediction_points = os.path.join(gdb_path, f"prediction_prob_points_{run_idx:03}")
prediction_raster = os.path.join(gdb_path, f"prediction_prob_raster_{run_idx:03}")
imp_table_tmp = os.path.join(gdb_path, f"var_imp_points_{run_idx:03}")
cls_table_tmp = os.path.join(gdb_path, f"class_points_{run_idx:03}")
...
with arcpy.EnvManager(
    ...
    arcpy.stats.Forest(
        ...
        features_to_predict=r"Link_to_rasterpoints", #<-- raster converted to point feature
        output_features=prediction_points,
        output_raster=None,
    ...
    arcpy.conversion.PointToRaster(
        in_features=prediction_points,
        value_field="PROB_02",
        out_rasterdataset=prediction_raster,
